...
const ModelProfileCategoryStep = ({ formData, handleOptionButtonClick, modelProfileCategories, getSelectedUserTypeLabel }) => (
  <div className="space-y-5">
    ...
    <div>
      <Label htmlFor="model_profile_category" className="mb-2 block font-medium text-gray-700">Seu perfil:</Label>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
        {modelProfileCategories.map(category => (
          <OptionButton 
            key={category.value} 
            value={category.value} 
            selectedValue={formData.model_profile_category} 
            onClick={(value) => handleOptionButtonClick('model_profile_category', value)}
          >
            {category.label}
          </OptionButton>
        ))}
...
export default ModelProfileCategoryStep;