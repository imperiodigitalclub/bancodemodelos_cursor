...
const ModelPhysicalTypeStep = ({ formData, handleOptionButtonClick, modelPhysicalTypes, getSelectedUserTypeLabel }) => (
    ...
    >
      ...
      <div>
        <p className="text-sm font-medium text-pink-700">
          Detalhes da Aparência, {getSelectedUserTypeLabel()}!
        </p>
        <p className="text-xs text-gray-600 mt-1">
          Qual seu tipo físico predominante?
        </p>
      </div>
    </div>
    
    <div>
      <Label htmlFor="model_physical_type" className="mb-2 block font-medium text-gray-700">Seu tipo físico:</Label>
      <div className="grid grid-cols-2 gap-2">
        {modelPhysicalTypes.map(type => (
          <OptionButton 
            key={type.value} 
            value={type.value} 
            selectedValue={formData.model_physical_type} 
            onClick={(value) => handleOptionButtonClick('model_physical_type', value)}
          >
            {type.label}
          </OptionButton>
        ))}
...
export default ModelPhysicalTypeStep;